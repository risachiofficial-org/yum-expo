...
const featuredImageUrl =
	"https://fwcsgvyqmolgmbupulmv.supabase.co/storage/v1/object/public/common/recipe/dhokla.jpeg";
		...
		const fetchRecipes = async () => {
			try {
				...
				if (e instanceof Error) {
					setError(
						e.message || "Failed to load recipes. Please try again later.",
					);
				} else {
	...
	return (
		<ScrollView className="flex-1 bg-gray-50" style={{ paddingTop: top }}>
			...
			<View className="px-4 mt-4 mb-6">
				<TouchableOpacity
					onPress={() => router.push("/recipe/5")}
					className="bg-purple-700 rounded-xl p-6 flex-row items-center justify-between shadow-md"
				>
					<View className="flex-1">
						<Text className="text-xs text-purple-200 bg-white/20 px-2 py-1 rounded-full self-start mb-1">
							Delicacies from Gujarat
						</Text>
						<Text className="text-2xl font-bold text-white">
							Gujarati Delights
						</Text>
						<TouchableOpacity className="mt-2 flex-row items-center">
							<Text className="text-purple-100 font-medium">
								Browse Recipes!
							</Text>
							<Ionicons
								name="arrow-forward"
